...
### 核心业务
- **房源管理**：项目 / 楼栋 / 房间三级结构，房间状态（空置 / 在租 / 配置中）流转，一房同时仅允许一份生效合同；**房屋历史租赁查询**——任一房间可查看历届租约完整履历（租赁人姓名 / 身份证 / 手机号、租赁时长、月租金、水电垃圾费率、押金退还明细、合同签订时间、退房时间）
...
- **缴费提醒**：① 系统自动——每月账单**到期日前 3 天**自动下发提醒（租金 / 水电 / 垃圾费），服务内置定时任务（每 6 小时检查，幂等不重复）；② 后台手动——账单页勾选未缴 / 逾期账单**批量发起催缴提醒**（管理员 / 操作员均可，操作员限授权项目）；③ 账单列表**回显提醒状态**（已提醒 / 未提醒、提醒时间、来源为自动或手动）；提醒通过**租客小程序**送达，租客点开提醒自动跳转对应账单页即可在线支付
