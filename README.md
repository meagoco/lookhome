...
### 宣传首页（Landing）
- **对外展示门户**：访问站点根路径（未登录）即进入公寓管理宣传首页，展示系统定位、核心卖点（智能管理 / 微信生态 / 私有化部署）与完整业务流程；页面顶部提供**登录按钮**，点击进入后台登录页；已登录用户访问根路径自动进入仪表盘
- 移动端自适应，作为项目对外展示与招商的入口页面
...
### 租客端小程序（miniprogram/ 目录）

| 能力 | 说明 |
|---|---|
| 微信一键登录 | `wx.login` → 后端 `code2session` 换取 openid；已绑定微信直接登录，未绑定则用注册手机号+身份证完成绑定 |
| 实名注册 | 姓名 + 手机号 + 身份证，默认**人工审核**（后台通过后可绑定房间/查看账单），预留自动核验开关 |
| 绑定 openid | 注册或登录时自动写入 `wechat_openid`，同一 openid 不可被多个账号占用 |
| 页面 | 登录/注册（含微信按钮）、我的、账单列表/详情、个人中心（申请退押金） |
| 服务器配置 | `miniprogram/config.js` 统一配置 apiBase；正式环境需**已备案 https 域名**并配置小程序 request 合法域名 |

### 微信支付（后台系统设置开通）

| 配置项 | 说明 |
|---|---|
| 小程序 AppID / AppSecret | 租客端小程序绑定（已配置后即启用微信登录） |
| 微信支付商户号 / API 密钥 / 证书 | 线上收款开通（需先申请微信支付商户号）；未配置时支付降级为「线下转账 + 后台确认」 |
| 多支付模板 | 可创建多套支付配置并按项目勾选绑定，一个或多个项目共用一套商户号 |
| 收款确认 | 默认**手工确认**，绑定商户号后可按需切换自动入账 |
| 安全回调 | APIv3 平台公钥验签 + 金额核对 + 防重放，未配置公钥一律拒绝回调 |

> 开通方式：后台「系统设置」填入对应参数 → 保存即生效，无需改代码。
