...
// 租赁历史全局搜索（按租客姓名/身份证/手机号；管理员全量，操作员限授权项目）
r.get('/history-search', auth, (req, res) => {
  const { name, id_card, phone } = req.query;
  const { cond, args } = grantedScope(req, 'r');
  let sql = `
    SELECT c.*, t.name AS tenant_name, t.phone AS tenant_phone, t.id_card_enc AS tenant_id_card,
      r.room_no, p.name AS property_name,
      (SELECT COALESCE(SUM(amount),0) FROM refunds rf WHERE rf.contract_id=c.id AND rf.status='done') AS refunded_amount,
      (SELECT COUNT(*) FROM refunds rf WHERE rf.contract_id=c.id AND rf.status='done') AS refund_count
    FROM contracts c
    JOIN tenants t ON t.id=c.tenant_id
    JOIN rooms r ON r.id=c.room_id
    ...
    WHERE 1=1${cond}`;
  const a = [...args];
  if (name) { sql += ' AND t.name LIKE ?'; a.push(`%${name}%`); }
  if (id_card) { sql += ' AND t.id_card_enc LIKE ?'; a.push(`%${id_card}%`); }
  if (phone) { sql += ' AND t.phone LIKE ?'; a.push(`%${phone}%`); }
