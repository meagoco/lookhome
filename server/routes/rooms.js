...
r.post('/', auth, (req, res) => {
  ...
  const prop = db.prepare('SELECT id FROM properties WHERE id=?').get(b.property_id);
  if (!prop) return res.status(400).json({ error: '项目不存在' });
...
r.delete('/:id', auth, (req, res) => {
  ...
  const cleaned = db.transaction(() => {
    ...
    if (bills.length) db.prepare(`DELETE FROM reminders WHERE bill_id IN (${bills.map(() => '?').join(',')})`).run(...bills);
