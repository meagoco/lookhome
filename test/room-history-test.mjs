...
// 全局租赁历史搜索（姓名/身份证/手机号）
r = await req('GET', `/contracts/history-search?name=${encodeURIComponent('张三')}`, adminTok);
const hs1 = Array.isArray(r.j) ? r.j : (r.j.rows || []);
check('按姓名搜到张三合同', hs1.length >= 1 && hs1.some(x => x.id === cid1), JSON.stringify(hs1));
r = await req('GET', `/contracts/history-search?id_card=${encodeURIComponent('110101199002022345')}`, adminTok);
const hs2 = Array.isArray(r.j) ? r.j : (r.j.rows || []);
check('按身份证搜到李四合同', hs2.length >= 1 && hs2.some(x => x.id === cid2), JSON.stringify(hs2));
r = await req('GET', `/contracts/history-search?phone=${`138${rnd}`}`, adminTok);
const hs3 = Array.isArray(r.j) ? r.j : (r.j.rows || []);
check('按手机号搜到张三合同', hs3.length >= 1 && hs3.some(x => x.id === cid1), JSON.stringify(hs3));
r = await req('GET', `/contracts/history-search?name=${encodeURIComponent('不存在的名字')}`, adminTok);
const hs4 = Array.isArray(r.j) ? r.j : (r.j.rows || []);
check('无匹配返回空', hs4.length === 0);
check('搜索结果含退还金额与状态', hs1.some(x => x.id === cid1 && x.refund_count === 1 && x.refunded_amount === 1400));
// 操作员权限：未授权搜索不到
r = await req('GET', `/contracts/history-search?name=${encodeURIComponent('张三')}`, opTok2);
const hs5 = Array.isArray(r.j) ? r.j : (r.j.rows || []);
check('未授权操作员搜索为空', hs5.length === 0);
