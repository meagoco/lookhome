...
const routes = [
  ...
  { path: '/reset', component: () => import('./views/Reset.vue') },
  {
    ...
    children: [
      ...
      { path: '/history', component: () => import('./views/HistorySearch.vue'), meta: { title: '租赁历史' } },
...
router.beforeEach((to) => {
  ...
  // 根路径：未登录显示宣传首页，已登录直接进后台
  if (to.path === '/') return token ? '/dashboard' : undefined;
  if (!token && to.path !== '/login' && to.path !== '/reset') return '/login';
  if (token && (to.path === '/login' || to.path === '/reset')) return '/dashboard';
